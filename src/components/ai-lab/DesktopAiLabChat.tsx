...
export default function DesktopAiLabChat() {
  ...
  return <>
    <div className="fixed inset-0 z-50 hidden bg-black/10 md:block" onClick={() => setOpen(false)} aria-hidden="true" />
    <section role="dialog" aria-modal="false" aria-label="AI Lab chat" className="fixed bottom-5 right-5 z-[51] hidden h-[min(720px,calc(100vh-2.5rem))] w-[min(440px,calc(100vw-2.5rem))] overflow-hidden rounded-2xl border border-border bg-background shadow-2xl md:flex md:flex-col">
      <header className="flex shrink-0 items-center justify-between border-b border-border/70 px-4 py-3"><div className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-emerald-500" /><span className="font-bold">AI Lab</span></div><div className="flex items-center gap-1"><button type="button" onClick={() => navigate("/ai-lab")} aria-label="Open full AI Lab" className="rounded-full p-2 text-muted-foreground hover:bg-muted"><ExternalLink className="h-4 w-4" /></button><button type="button" onClick={() => setOpen(false)} aria-label="Close AI Lab chat" className="rounded-full p-2 text-muted-foreground hover:bg-muted"><X className="h-4 w-4" /></button></div></header>
      <div className="min-h-0 flex-1"><AiLabChat messages={messages} onSubmit={submit} compareStateByMessageId={compareStateByMessageId} onLookbackChange={(id, days) => setLookbacks((prev) => ({ ...prev, [id]: days }))} onFeedback={(id, value) => setMessages((prev) => prev.map((m) => m.id === id ? { ...m, feedback: value } : m))} onClarificationSelect={selectClarification} /></div>
    </section>
  </>;
}
