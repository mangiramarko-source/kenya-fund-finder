            ...
            <div className="prose prose-sm max-w-none text-foreground leading-relaxed space-y-4">
              {article.content ? (
                article.content.split("\n").filter(Boolean).map((paragraph, i) => (
                  <p key={i} className="sm:text-base text-muted-foreground leading-relaxed font-normal font-sans mx-[2px] my-0 px-0 py-0 border-8 border-none text-justify pl-[10px] pr-[10px] text-lg">{decodeHtmlEntities(paragraph)}</p>
                ))
              ) : (
