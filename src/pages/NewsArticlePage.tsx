...
const NewsArticlePage = () => {
  ...
  return (
    <div className="md:max-w-3xl md:mx-auto md:py-8 md:px-6">
      ...
      <div className="md:hidden sticky top-0 z-40 bg-background/90 backdrop-blur-md border-b border-border px-3 py-2 flex items-center justify-between">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" aria-label="Share article" className="h-9 w-9 -mr-1">
            ...
            <div className="prose prose-sm max-w-none text-foreground leading-relaxed space-y-4">
              {article.content ? (
                article.content.split("\n").filter(Boolean).map((paragraph, i) => (
                  <p key={i} className="sm:text-base text-muted-foreground leading-relaxed font-normal font-sans mx-[2px] my-0 px-0 py-0 border-8 border-none pl-[10px] pr-[10px] text-left text-xl">{decodeHtmlEntities(paragraph)}</p>
                ))
              ) : (
