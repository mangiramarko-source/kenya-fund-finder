              ...
              <table className="w-full text-sm">
                ...
                <thead>
                  <tr className="bg-muted/70 text-xs">
                    ...
                    <th className="text-left px-3 py-3"><SortHeader label="Currency" field="name" sortKey={sortKey} onToggle={toggleSort} /></th>
                    <th className="text-right px-3 py-3"><SortHeader label="Rate (KES)" field="rate" sortKey={sortKey} onToggle={toggleSort} className="justify-end" /></th>
                    <th className="text-right pr-5 px-3 py-3 font-semibold text-muted-foreground">Change</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredRates.map((r, i) => (
                    <tr key={r.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-3 py-3 text-right">
                        <span className="font-bold text-accent text-base tabular-nums">{Number(r.rate).toFixed(2)}</span>
                      </td>
                  ...
                  {filteredRates.length === 0 && <EmptyRow colSpan={4} search={search} onClear={() => setSearch("")} />}
                </tbody>
              </table>
            </div>
          )}
            ...
            <div className="rounded-xl border border-border overflow-hidden bg-card">
              <table className="w-full text-sm">
                ...
                <thead>
                  <tr className="bg-muted/70 text-xs">
                    <th className="text-left pl-5 pr-2 py-3 font-semibold text-muted-foreground">#</th>
                    <th className="text-left px-3 py-3"><SortHeader label="Commodity" field="name" sortKey={sortKey} onToggle={toggleSort} /></th>
                    <th className="text-center px-2 py-3 font-semibold text-muted-foreground">Unit</th>
                    <th className="text-right px-3 py-3"><SortHeader label="Price" field="price" sortKey={sortKey} onToggle={toggleSort} className="justify-end" /></th>
                    <th className="text-right pr-5 px-3 py-3 font-semibold text-muted-foreground">Change</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredCommodities.map((c, i) => (
                    <tr key={c.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-3 py-3 text-right">
                        <span className="font-bold text-accent text-base tabular-nums">{Number(c.price).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                      </td>
                      <td className="pr-5 px-3 py-3 text-right">
                  ...
                  {filteredCommodities.length === 0 && <EmptyRow colSpan={5} search={search} onClear={() => setSearch("")} />}
                </tbody>
              </table>
            </div>
          )}
