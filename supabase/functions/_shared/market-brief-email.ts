...
export interface MarketBriefEmailData {
  ...
  newsSummary?: MarketBriefNewsSummaryItem[]; news?: MarketBriefNewsItem[]; discoveryActions?: MarketBriefDiscoveryAction[]; ctaUrl: string; watchlistUrl?: string;
}
...
function newsSummary(items?: MarketBriefNewsSummaryItem[]) {
  const selected = items?.slice(0, 5) ?? [];
  if (!selected.length) return "";
  const body = `<div style="font-family:Arial,Helvetica,sans-serif;font-size:10px;line-height:16px;font-weight:700;letter-spacing:1px;color:#17734c">STORED MARKET NEWS · SAMPLE</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-top:10px">${selected.map((item, index) => `<tr><td style="${index ? "padding-top:14px;margin-top:14px;border-top:1px solid #ece9e1;" : ""}"><div style="font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:16px;font-weight:700;letter-spacing:.4px;color:#697483">${escapeHtml(item.source)} · ${escapeHtml(item.published_at)}</div>${item.url ? `<a href="${escapeHtml(item.url)}" style="display:block;padding-top:4px;font-family:Arial,Helvetica,sans-serif;font-size:16px;line-height:21px;font-weight:700;color:#202938;text-decoration:none">${escapeHtml(item.headline)}</a>` : `<div style="padding-top:4px;font-family:Arial,Helvetica,sans-serif;font-size:16px;line-height:21px;font-weight:700;color:#202938">${escapeHtml(item.headline)}</div>`}<div style="padding-top:6px;font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:19px;color:#697483"><strong style="color:#3d4a5a">Why it matters:</strong> ${escapeHtml(item.why_it_matters)}</div><div style="padding-top:4px;font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:19px;color:#697483"><strong style="color:#3d4a5a">Takeaway:</strong> ${escapeHtml(item.takeaway)}</div></td></tr>`).join("")}</table>`;
  return `${title("News summary")}${panel(body, "20px")}`;
}
  ...
  newsSummary: [
    {
      ...
      category: "Markets",
      headline: "Sample banks lead NSE turnover as large caps move",
      why_it_matters: "Banking names shaped most of the fictional exchange activity used in this preview.",
      takeaway: "Watch liquidity and large-cap breadth, not a single index call.",
    ...
    },
    {
      ...
      published_at: "Aug 24, 2026",
      category: "FX",
      headline: "Sample shilling holds steady against the dollar",
      why_it_matters: "FX stability affects fund, importer, and household cost assumptions in the demo brief.",
      takeaway: "USD/KES stayed within the stored sample range used in today's preview.",
      url: "https://kenyafundfinder.com/news",
    },
    {
      ...
      category: "Funds",
      headline: "Sample money-market funds keep yield focus",
      why_it_matters: "Fund yield updates give context for cash investors comparing published returns.",
      takeaway: "Use dated fact sheets and source labels before treating any yield as current.",
      url: "https://kenyafundfinder.com/news",
    },
    {
      ...
      headline: "Sample commodity notes frame import-cost pressure",
      why_it_matters: "Commodity moves can affect inflation-sensitive sectors without proving stock-price impact.",
      takeaway: "Keep commodity notes clearly dated and separate from daily equity breadth.",
      url: "https://kenyafundfinder.com/news",
    },
  ],
  movers: [
