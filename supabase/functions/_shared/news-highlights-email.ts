...
function blendPrimaryText(escapedText: string): string {
  return `<div class="gmail-blend-screen"><div class="gmail-blend-difference">${escapedText}</div></div>`;
}

const title = (value: string) =>
  `<tr><td bgcolor="${contentBg}" style="padding:28px 16px 10px;background-color:${contentBg};background:linear-gradient(${contentBg},${contentBg});font-family:Arial,Helvetica,sans-serif;font-size:20px;line-height:26px;font-weight:700;color:${primaryText}">${blendPrimaryText(escapeHtml(value))}</td></tr>`;

function panel(content: string, padding = "18px") {
  return `<tr><td bgcolor="${contentBg}" style="padding:0 16px;background-color:${contentBg};background:linear-gradient(${contentBg},${contentBg})"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" bgcolor="${cardBg}" style="background-color:${cardBg};background:linear-gradient(${cardBg},${cardBg});border:1px solid ${border};border-radius:14px"><tr><td bgcolor="${cardBg}" style="padding:${padding};background-color:${cardBg};background:linear-gradient(${cardBg},${cardBg})">${content}</td></tr></table></td></tr>`;
}
...
function storyCard(story: NewsHighlightStory, featured = false) {
  ...
  const ticker = story.ticker
    ? `<span style="display:inline-block;margin-left:8px;padding:4px 8px;background-color:${contentBg};background:linear-gradient(${contentBg},${contentBg});border:1px solid ${border};border-radius:6px;font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:14px;font-weight:700;color:${primaryText}">${escapeHtml(story.ticker)}${story.price ? ` <span style="font-weight:400;color:${mutedText}">${escapeHtml(story.price)}</span>` : ""}${story.change ? ` <span style="color:${story.change.startsWith("-") ? red : accentGreen}">${escapeHtml(story.change)}</span>` : ""}</span>`
    : "";
  const meta = [story.source, formatDisplayDate(story.published_at)].filter(Boolean).join(" · ");
  return `<tr><td bgcolor="${contentBg}" style="${featured ? "" : "padding:0 0 14px;"}background-color:${contentBg};background:linear-gradient(${contentBg},${contentBg})"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" bgcolor="${bg}" style="background-color:${bg};background:linear-gradient(${bg},${bg});border:1px solid ${border};border-radius:12px"><tr><td bgcolor="${bg}" style="padding:${featured ? "22px" : "18px"};background-color:${bg};background:linear-gradient(${bg},${bg})"><div><span style="display:inline-block;padding:5px 10px;background-color:${darkGreenAccent};border-radius:6px;font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:14px;font-weight:700;letter-spacing:.7px;color:${primaryText}">${escapeHtml(story.category)}</span>${ticker}</div>${meta ? `<div style="padding-top:14px;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:16px;color:${mutedText}">${escapeHtml(meta)}</div>` : ""}<div style="padding-top:8px;font-family:Arial,Helvetica,sans-serif;font-size:${featured ? "20px" : "17px"};line-height:${featured ? "26px" : "23px"};font-weight:700;color:${primaryText}">${blendPrimaryText(escapeHtml(story.headline))}</div><div style="padding-top:8px;font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:22px;color:${secondaryText}">${escapeHtml(story.summary)}</div><a href="${escapeHtml(story.url)}" style="display:inline-block;padding-top:12px;font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:18px;font-weight:700;color:${accentGreen};text-decoration:none">Read more -&gt;</a></td></tr></table></td></tr>`;
}

function topStories(items: NewsHighlightStory[]) {
  const selected = items.slice(0, 5);
  if (!selected.length) return "";
  return `${title("Top stories")}<tr><td bgcolor="${contentBg}" style="padding:0 16px;background-color:${contentBg};background:linear-gradient(${contentBg},${contentBg})"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" bgcolor="${contentBg}" style="background-color:${contentBg};background:linear-gradient(${contentBg},${contentBg})">${selected.map((item) => storyCard(item)).join("")}</table></td></tr>`;
}

function insights(items: NewsHighlightInsight[]) {
  if (!items.length) return "";
  const rows = items.slice(0, 4).map((item, index) => `<tr><td bgcolor="${cardBg}" style="${index ? `padding-top:14px;border-top:1px solid ${border};` : ""}background-color:${cardBg};background:linear-gradient(${cardBg},${cardBg})"><div style="font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:16px;font-weight:700;letter-spacing:.9px;color:${accentGreen}">${escapeHtml(item.label)}</div><div style="padding-top:5px;font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:21px;color:${primaryText}">${escapeHtml(item.detail)}</div></td></tr>`).join("");
  return `${title("Why it matters")}${panel(`<table role="presentation" width="100%" cellspacing="0" cellpadding="0" bgcolor="${cardBg}" style="background-color:${cardBg};background:linear-gradient(${cardBg},${cardBg})">${rows}</table>`, "20px")}`;
}

function companyWatch(items: CompanyWatchItem[]) {
  if (!items.length) return "";
  const rows = items.map((item, index) => `<tr><td bgcolor="${cardBg}" style="${index ? `padding-top:14px;border-top:1px solid ${border};` : ""}background-color:${cardBg};background:linear-gradient(${cardBg},${cardBg})"><div style="font-family:Arial,Helvetica,sans-serif;font-size:16px;line-height:21px;font-weight:700;color:${primaryText}">${item.ticker ? `<span style="color:${accentGreen}">${escapeHtml(item.ticker)}</span> ` : ""}${escapeHtml(item.company)}${item.tag ? ` <span style="font-size:10px;line-height:14px;font-weight:700;letter-spacing:.7px;color:${blue}">${escapeHtml(item.tag)}</span>` : ""}</div><div style="padding-top:5px;font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:21px;color:${secondaryText}">${escapeHtml(item.summary)}</div></td></tr>`).join("");
  return `${title("Company watch")}${panel(`<table role="presentation" width="100%" cellspacing="0" cellpadding="0" bgcolor="${cardBg}" style="background-color:${cardBg};background:linear-gradient(${cardBg},${cardBg})">${rows}</table>`, "20px")}`;
}

function policyWatch(items: PolicyWatchItem[]) {
  if (!items.length) return "";
  const rows = items.map((item, index) => `<tr><td bgcolor="${cardBg}" style="${index ? `padding-top:14px;border-top:1px solid ${border};` : ""}background-color:${cardBg};background:linear-gradient(${cardBg},${cardBg})">${item.category ? `<div style="font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:15px;font-weight:700;letter-spacing:.8px;color:${accentGreen}">${escapeHtml(item.category)}</div>` : ""}<div style="padding-top:${item.category ? "5px" : "0"};font-family:Arial,Helvetica,sans-serif;font-size:16px;line-height:21px;font-weight:700;color:${primaryText}">${escapeHtml(item.headline)}</div><div style="padding-top:5px;font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:21px;color:${secondaryText}">${escapeHtml(item.summary)}</div></td></tr>`).join("");
  return `${title("Economy / policy watch")}${panel(`<table role="presentation" width="100%" cellspacing="0" cellpadding="0" bgcolor="${cardBg}" style="background-color:${cardBg};background:linear-gradient(${cardBg},${cardBg})">${rows}</table>`, "20px")}`;
}

function featured(story: NewsHighlightStory) {
  return `${title("Editor's pick")}<tr><td bgcolor="${contentBg}" style="padding:0 16px;background-color:${contentBg};background:linear-gradient(${contentBg},${contentBg})"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" bgcolor="${contentBg}" style="background-color:${contentBg};background:linear-gradient(${contentBg},${contentBg})">${storyCard(story, true)}</table></td></tr>`;
}
...
export function renderNewsHighlightsEmail(data: NewsHighlightsEmailData): RenderedNewsHighlightsEmail {
  const hero = `<tr><td bgcolor="${contentBg}" style="background-color:${contentBg};background:linear-gradient(${contentBg},${contentBg})"><img src="${NEWS_HIGHLIGHTS_HERO_URL}" width="600" alt="${NEWS_HIGHLIGHTS_HERO_ALT}" style="display:block;width:100%;max-width:600px;height:auto;border:0;"></td></tr>`;
  const demoNote = data.demo ? `<tr><td bgcolor="${contentBg}" style="padding:16px 16px 0;background-color:${contentBg};background:linear-gradient(${contentBg},${contentBg})"><span style="display:inline-block;padding:5px 10px;background-color:${darkGreenAccent};border-radius:999px;font-family:Arial,Helvetica,sans-serif;font-size:10px;line-height:13px;font-weight:700;letter-spacing:.8px;color:${primaryText}">DEMO · SAMPLE DATA</span></td></tr>` : "";
  ...
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><meta name="x-apple-disable-message-reformatting"><title>KenyaFundFinder News Highlights</title><style>body{margin:0;padding:0;background-color:${outerBg};color:${primaryText};-webkit-text-size-adjust:100%}@media only screen and (max-width:480px){.email-shell{width:100%!important}}u + .body .gmail-blend-screen{background:#000;mix-blend-mode:screen}u + .body .gmail-blend-difference{background:#000;mix-blend-mode:difference}</style></head><body class="body" bgcolor="${outerBg}" style="margin:0;padding:0;background-color:${outerBg};background:linear-gradient(${outerBg},${outerBg});color:${primaryText}"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" bgcolor="${outerBg}" style="width:100%;margin:0;padding:0;background-color:${outerBg};background:linear-gradient(${outerBg},${outerBg})"><tr><td align="center" bgcolor="${outerBg}" style="padding:20px 8px;background-color:${outerBg};background:linear-gradient(${outerBg},${outerBg})"><table role="presentation" class="email-shell" width="100%" cellspacing="0" cellpadding="0" bgcolor="${contentBg}" style="max-width:600px;width:100%;background-color:${contentBg};background:linear-gradient(${contentBg},${contentBg});border:1px solid ${border};border-radius:16px;overflow:hidden">${hero}${demoNote}${topStories(data.topStories)}${insights(data.whyItMatters)}${companyWatch(data.companyWatch)}${policyWatch(data.policyWatch)}${featured(data.featuredStory)}<tr><td bgcolor="${contentBg}" style="padding:28px 16px 0;background-color:${contentBg};background:linear-gradient(${contentBg},${contentBg})"><table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr><td align="center" bgcolor="${accentGreen}" style="background-color:${accentGreen};background:linear-gradient(${accentGreen},${accentGreen});border-radius:8px"><a href="${escapeHtml(data.ctaUrl)}" style="display:block;padding:14px 20px;font-family:Arial,Helvetica,sans-serif;font-size:13px;font-weight:700;letter-spacing:.65px;color:${outerBg};text-decoration:none">VIEW MORE MARKET NEWS</a></td></tr></table></td></tr><tr><td bgcolor="${outerBg}" style="padding:28px 20px;background-color:${outerBg};background:linear-gradient(${outerBg},${outerBg});border-top:1px solid ${border};font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:19px;color:${mutedText}"><strong style="color:${primaryText}">${blendPrimaryText("KenyaFundFinder")}</strong><br>Kenyan markets, explained simply.<br><br>Market information is provided for informational purposes only and should not be considered financial advice.<br><br><a href="${escapeHtml(preferencesUrl)}" style="color:${accentGreen};text-decoration:none">Email preferences</a> · <a href="${escapeHtml(unsubscribeUrl)}" style="color:${accentGreen};text-decoration:none">Unsubscribe</a> · <a href="https://kenyafundfinder.com" style="color:${mutedText};text-decoration:none">Privacy</a> · <a href="https://kenyafundfinder.com" style="color:${mutedText};text-decoration:none">KenyaFundFinder.com</a></td></tr></table></td></tr></table></body></html>`;
