...
Deno.serve(async (request) => {
  ...
  const preferencePatch = verified.scope === "market_brief"
    ...
    : { market_brief_email: false, price_alert_email: false };
  ...
  const { data: existing } = await supabase
    ...
    .maybeSingle();
  if (!existing) {
    await supabase.from("communication_suppressions").insert({
